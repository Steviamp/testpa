...
namespace Login_and_Register
{
    public partial class frmDashboard : Form
    {
        ...
        private async Task RefreshInfo()
        {
            try
            {
                Log.Warning(">>> ENTER RefreshInfo()");

                CommonRequest MyItem = new CommonRequest { cashierNr = cashierNr, host = host };
                Log.Warning(">>> Request built | Cashier={Cashier} Host={Host}", MyItem.cashierNr, MyItem.host);

                CustomResponse MyCustomResponse = await WinPanelAPIService.GetRefreshInfo(MyItem);
                Log.Warning(">>> Response arrived | Code={Code} DataLen={Len}", MyCustomResponse?.HttpResponseCode, MyCustomResponse?.Data?.Length ?? -1);

                if (MyCustomResponse.HttpResponseCode == 200)
                {
                    Log.Warning(">>> In 200-branch");
                    ...
                    {
                        Log.Information("RefreshInfo: NBGTicket found. From={From}, To={To}", info.NBGTicket.slotFrom, info.NBGTicket.slotTo);
                    }
