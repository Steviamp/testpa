...
namespace Login_and_Register
{
    public partial class frmDashboard : Form
    {
        ...
        private bool nbgTicketPopupShown = false;
        private string? _lastShownTicketKey = null;
        ...
        private async Task RefreshInfo()
        {
            try
            {
                ...
                {
                    var raw = MyCustomResponse.Data;
                    Log.Information("RefreshInfo raw payload: {Payload}",
                        string.IsNullOrEmpty(raw) ? "<empty>"
                                                  : raw.Substring(0, Math.Min(400, raw.Length)));
                    if (string.IsNullOrWhiteSpace(MyCustomResponse.Data))
                    {
                        Log.Error("RefreshInfo: empty response body (200 OK) | Cashier={Cashier} Host={Host}", cashierNr, host);
                        Log.Warning(">>> RETURN because Data is empty");
                        return;
                    }

                    var info = JsonConvert.DeserializeObject<GetRefreshInfoResponse>(MyCustomResponse.Data);
                    Log.Warning(">>> Deserialized info is {State}", info == null ? "NULL" : "NOT NULL");
                    if (info == null)
                    {
                        Log.Error("RefreshInfo: failed to deserialize response. Raw payload (first 400 chars): {Payload}",
                                  MyCustomResponse.Data.Substring(0, Math.Min(400, MyCustomResponse.Data.Length)));
                        return;
                    }

                    // Δημιουργία unique key για το τρέχον ticket

                    string? currentTicketKey = null;

                    if (!string.IsNullOrWhiteSpace(info.ticketNr))
                    {
                        currentTicketKey = info.ticketNr;
                    }
                    else if (info.NBGTicket != null &&
                             !string.IsNullOrWhiteSpace(info.NBGTicket.slotFrom) &&
                             !string.IsNullOrWhiteSpace(info.NBGTicket.slotTo))
                        currentTicketKey = $"{info.NBGTicket.slotFrom}|{info.NBGTicket.slotTo}";

                    if (info.NBGTicket == null)
                    {
                        Log.Information("RefreshInfo: NBGTicket == null");
                        nbgTicketPopupShown = false;
                        _lastShownTicketKey = null;
                    }
                    else if (!string.IsNullOrWhiteSpace(currentTicketKey) &&
                     currentTicketKey != _lastShownTicketKey)
                    {
                        Log.Information("RefreshInfo: New ticket detected. Current={Current}, Last={Last}",
                                       currentTicketKey, _lastShownTicketKey);
                        // Νέο ticket - reset το flag
                        nbgTicketPopupShown = false;
                    }

                    if (info.NBGTicket != null)
                    {
                        Log.Information("RefreshInfo: NBGTicket found. From={From}, To={To}",
                                       info.NBGTicket.slotFrom, info.NBGTicket.slotTo);
                    }
                    AssignTextboxes(info);

                    if (!nbgTicketPopupShown &&
                        !_popupOpen &&
                    info.NBGTicket != null &&
                    !string.IsNullOrEmpty(info.NBGTicket.slotFrom) &&
                    !string.IsNullOrEmpty(info.NBGTicket.slotTo) &&
                    !string.IsNullOrWhiteSpace(currentTicketKey))
                    {
                        Log.Information("RefreshInfo: Showing popup for ticket {TicketKey}", currentTicketKey);

                        string msg = $"Υπάρχει διαθέσιμο ραντεβού για το χρονικό διάστημα " +
                                     $"Από: {info.NBGTicket.slotFrom} Έως: {info.NBGTicket.slotTo}";
                        ...
                        if (this.IsHandleCreated)
                        {
                            this.BeginInvoke(new Action(async () =>
                            {
                                try
                                {
                                    nbgTicketPopupShown = true;
                                    _lastShownTicketKey = currentTicketKey;
                                    _popupOpen = true;

                                    var popup = new frmConfirmDialog(msg);
                                    var result = popup.ShowDialog();
                                    bool accepted = result == DialogResult.OK && popup.Accepted;
                                    Log.Warning(">>> Popup closed | Accepted={Accepted} DialogResult={DR}", accepted, result);

                                    var decisionRequest = new CommonRequest
                                    {
                                        cashierNr = cashierNr,
                                        host = host,
                                        nbgTicketDisplayed = true,
                                        nbgTicketAccepted = accepted
                                    };
                                    await WinPanelAPIService.GetRefreshInfo(decisionRequest);
                                    Log.Information("NBGTicket decision sent. Displayed=true, Accepted={Accepted}", accepted);
                                }
                                catch (Exception ex)
                                {
                                    Log.Error("Popup/Decision exception: {Ex}", ex.ToString());
                        ...
                        {
                            Log.Warning("RefreshInfo: handle not created; skipping popup.");
                        }
                    }
                }
                else
                {
                    ...
                    var errRaw = MyCustomResponse.Data;
                    Log.Warning("Non-200 response | Code={Code} Body(first400)={Body}",
                        MyCustomResponse.HttpResponseCode,
                        string.IsNullOrEmpty(errRaw) ? "<empty>" : errRaw.Substring(0, Math.Min(400, errRaw.Length)));

                    if (!string.IsNullOrEmpty(MyCustomResponse.Data))
                    {
                        try
                        {
                            var MyErrorResponse = JsonConvert.DeserializeObject<ErrorResponse>(MyCustomResponse.Data);
                            ErrorMsg = MyErrorResponse?.Error ?? "";
                        }
                        catch
                        {
                            ErrorMsg = MyCustomResponse.Data;
                        }
                    }

                    if (!string.IsNullOrEmpty(ErrorMsg) &&
                        (ErrorMsg.Contains("time") || ErrorMsg.Contains("connection")))
                    {
                        var dlg = new frmException("Connection to server lost. Press Connect to try restoring connection or X to exit the application.");
                        dlg.ShowDialog();
                        await RefreshInfo();
                    }
                    else
                    {
                        MessageBox.Show($"{MyCustomResponse.HttpResponseCode} {ErrorMsg}");
                    }

                    Log.Error("Dashboard RefreshInfo error | Cashier={Cashier} Host={Host} Code={Code} Err={Err}",
                        MyItem.cashierNr, MyItem.host, MyCustomResponse.HttpResponseCode, ErrorMsg);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (MyGetRefreshInfoResponse == null) return;
            try
            {
